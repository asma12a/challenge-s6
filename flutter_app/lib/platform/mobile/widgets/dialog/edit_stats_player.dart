...
class EditStatsPlayerDialog extends StatefulWidget {
  ...
  const EditStatsPlayerDialog({super.key, required this.eventId, required this.player, this.onRefresh});
...
class _EditStatsPlayerDialogState extends State<EditStatsPlayerDialog> {
  ...
  Future<void> _initializeRatings() async {
    try {
      ...
      final labels = await statLabelsService.getStatLabelsBySport(event!.sport.id);
      setState(() {
        statLabels = labels;
      });
  ...
  Future<void> _loadEventRatings() async {
    try {
      ...
      final existingRatings = await statLabelsService.getUserStatByEvent(event!.id!, _player.userID);
      setState(() {
  ...
  void _saveUserStat() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ajout des notes avec succées')),
      );
